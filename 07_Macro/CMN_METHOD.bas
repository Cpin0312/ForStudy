...
' パラメタ : 対象ID名、項番名、シート名
' 戻り値   : 修正後文字列
Public Function setListByID(ByVal strInput As String, koubanName as String, Optional sheetName as String )

    If isCellExist(strInput) = True Then

        ' 処理種別リストを取得
        Dim ListDict As Object: Set ListDict = getListDictionary(sheetName, strInput)
        ' 項番数を取得
        Dim cntTotal As Integer: cntTotal = getCountCase(koubanName, 2)
        ' 項番セル位置を取得
        Dim cellKoban As Range: Set cellKoban = Range(searchCell(koubanName))
        ' 対象項目IDを取得
        Dim targetRange As Range: Set targetRange = Range(searchCell(strInput))

        ' 開始位置を取得
        Dim strRangeStart As String: strRangeStart = Cells(cellKoban.Row + 2, targetRange.Column).Address
        ' 終了位置を取得
        Dim strRangeEnd As String: strRangeEnd = Cells(cellKoban.Row + 2 + cntTotal - 1, targetRange.Column).Address
        ' 開始終了位置を設定
        Dim rangeArea As String: rangeArea = strRangeStart + " : " + strRangeEnd
        ' 開始終了セルを設定
        Dim cell As Range: Set cell = Range(rangeArea)
...
' ファイルの削除
' 複数削除する場合、【*】をつける
Public Function deleteFiles(ByVal inputPath As String) As String

    Dim fso As FileSystemObject
    Set fso = New FileSystemObject
    Call fso.DeleteFile(inputPath, True) ' すべてのファイルを削除
...
' ファイルの削除
' 複数削除する場合、【*】をつける
Public Function createFolder(ByVal inputPath As String) As String

    Dim fso As FileSystemObject
    Set fso = New FileSystemObject
	Call objFso.CreateFolder(inputPath)
